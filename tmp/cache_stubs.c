...
_res->man = NULL;
  ...
  _vres = camlidl_c2ml_cache_cache__t(&_res, _ctx);
  camlidl_free(_ctx);
  return _vres;
}

value camlidl_cache_arity(
	value _v_cache)
{
  cache__t cache; /*in*/
  int _res;
  ...
  struct camlidl_ctx_struct _ctxs = { CAMLIDL_TRANSIENT, NULL };
  camlidl_ctx _ctx = &_ctxs;
  camlidl_ml2c_cache_cache__t(_v_cache, &cache, _ctx);
  /* begin user-supplied calling sequence */
_res = cache->arity;
  /* end user-supplied calling sequence */
  _vres = Val_int(_res);
