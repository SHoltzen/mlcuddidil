...
_res->man = NULL;
  ...
  _vres = camlidl_c2ml_hash_hash__t(&_res, _ctx);
  camlidl_free(_ctx);
  return _vres;
}

value camlidl_hash_arity(
	value _v_hash)
{
  hash__t hash; /*in*/
  int _res;
  ...
  struct camlidl_ctx_struct _ctxs = { CAMLIDL_TRANSIENT, NULL };
  camlidl_ctx _ctx = &_ctxs;
  camlidl_ml2c_hash_hash__t(_v_hash, &hash, _ctx);
  /* begin user-supplied calling sequence */
_res = hash->arity;
  /* end user-supplied calling sequence */
  _vres = Val_int(_res);
